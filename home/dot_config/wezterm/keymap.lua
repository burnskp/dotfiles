...
function M.config(config)
  ...
  if wezterm.target_triple:find("darwin") ~= nil then
    ...
    table.insert(keys, { key = "-", mods = "CMD", action = act.DecreaseFontSize })
    table.insert(keys, { key = "=", mods = "CMD", action = act.ResetFontSize })
    table.insert(keys, { key = "q", mods = "CMD", action = act.QuitApplication })
    table.insert(keys, { key = "w", mods = "CMD", action = act.CloseCurrentPane({ confirm = true }) })
  end
